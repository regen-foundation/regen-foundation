<script lang="ts">
	...
	let { headerText, buttonText, successText, isFooter=false } = $props();
			...
			>
				<div class="flex  {isFooter ? '' : 'gap-4'}">
					...
					<button
						...
						type="submit"
						class="{isFooter ? 'bg-green text-field px-4' : 'text-green'}  cursor-pointer text-lg font-bold hover:underline">{buttonText}</button
					>
				</div>
			</form>
		</div>
